...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def write_error(self, status_code, **kwargs):
        ...
        msg = {'to':'jfw@gbicom.org','subject':'CRM错误','body':"""<html>
                         <title>%s</title>
                         <body>
                            <h1>Author:%s</h1>
                            <h2>Error</h2>
                       ...
                       </html>""" % (error,RedisCache.hget('Cache:Right.User',self.get_secure_cookie("user")), error, 
                                    trace_info, request_info)}
        RightsCache.lpush('mailqueue',msg)
...
class RestHandler(BaseHandler):
    ...
    def prepare(self):
        ...
        self.SortOrder = self.get_argument('order',None)
        if self.SortOrder:
            self.SortOrder = asc if self.SortOrder == 'asc' else desc
